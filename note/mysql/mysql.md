...
```go
func updateTaskStatusPessimistic(taskID string) error {
    tx, _ := db.Begin()
    defer tx.Rollback()
    ...
    _, err = tx.Exec("UPDATE tasks SET status = 'done' WHERE id = ?", taskID)
    if err != nil {
        return err
    }
    return tx.Commit()
...
```go
// 先在 tasks 表增加 version 字段：
ALTER TABLE tasks ADD COLUMN version INT DEFAULT 0;

func updateTaskStatusOptimistic(taskID string) error {
    // 1. 读取当前版本号
    var version int
    err := db.QueryRow("SELECT version FROM tasks WHERE id = ?", taskID).Scan(&version)
    if err != nil {
        return err
    }
    ...
    // 2. 更新时比较版本号
    res, err := db.Exec(
        "UPDATE tasks SET status = 'done', version = version + 1 WHERE id = ? AND version = ?",
        taskID, version,
    )
    if err != nil {
        return err
    ...
    if rows == 0 {
        // 版本号被其他请求改过了，重试或报错
        return fmt.Errorf("update conflict, please retry")
    }
    return nil
}
```
